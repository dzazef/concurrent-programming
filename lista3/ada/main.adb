...
procedure Main is
    ...
    type Machine_State is (Working, Broken);
    type Machine_In_Repair is (True, False);
    subtype Operator is Integer range 0..1;
    subtype Task_Int is Integer range 0 .. Parameters.Max_Tasks;
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
     Mode : Simulator_Mode := Talkative;
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
    procedure Calculate_Task(t: in out Task_Record; state: Machine_State) is
    begin
...
-- MACHINES
   ...
   task type Machine_Type is
        ...
        entry Backdoor;
        entry GetRepairStatus(S : out Machine_In_Repair);
   end Machine_Type;

    task body Machine_Type is
       Op : Operator;
       State : Machine_State;
       RepairState : Machine_In_Repair;
        ...
        loop
            select           
               accept Calculate ( Item : in out Task_Record) do
                    ...
                    if (FRand.Random(Gen) < Parameters.Break_Probability) then
                        State := Broken;
                        Print_Info_If_Talkative("MACHINE: Broken");
                     end if;
                    Calculate_Task(Item, State);
               end Calculate;
            or
               accept Backdoor do
                  State := Working;
                  RepairState := False;
                  Print_Info_If_Talkative("MACHINE: Repaired");
               end Backdoor;
            or
               accept GetRepairStatus(S : out Machine_In_Repair) do
                  S := RepairState;
               end GetRepairStatus;  
        end select;
        end loop;
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --    
    task type Service_Worker_Type is
        entry Start(ID1 : in Integer);
        entry Repair(Machine : Machine_Type);
    end Service_Worker_Type;
    
     task type Service_Type is 
        entry Start;
        entry Repair(Machine : Machine_Type);
    end Service_Type;

    task type Feedback_Type is
        entry Feedback(ID : Integer);
    end Feedback_Type;

    Feedback : Feedback_Type;
    Service : Service_Type;
    type Service_Worker_Array_Type is array (0 .. Parameters.Service_Workers-1) of Service_Worker_Type;
    Service_Worker_Array : Service_Worker_Array_Type;

-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --     
    task body Feedback_Type is
    begin 
        loop
            accept Feedback(ID : Integer) do
                Print_Info_If_Talkative("SERVICE: Feedback from service worker:" & Integer'Image(ID));
            end Feedback;
        end loop;
    end Feedback_Type;

-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --     
    task body Service_Worker_Type is
        ID : Integer;
    begin
        ...
        loop
            accept Repair(Machine : Machine_Type) do
                Print_Info_If_Talkative("SERVICE WORKER" & Integer'Image(ID) & ": Repairing machine");
                delay Parameters.Service_Worker_Speed;
                Machine.Backdoor;
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --     
    task body Service_Type is
        MachineRepairState : Machine_In_Repair;
    begin
        accept Start;
        loop
            accept Repair(Machine : Machine_Type) do
                Machine.GetRepairStatus(MachineRepairState);
                if (MachineRepairState = True) then
                    Print_Info_If_Talkative("SERVICE: Request ignored");
                else
                    Print_Info_If_Talkative("SERVICE: Request accepted");
                    Service_Worker_Array(Gen_Int(Parameters.Service_Workers)).Repair(Machine);
                end if;
            end Repair;
        end loop;
    end Service_Type;
        ...
        loop
            ...
            if (Worker_Mode = Patient) then
                loop
                    ...
                    Machine_ID := Gen_Int(Parameters.Machines);
                    Machine_Set(Current_Task.Op)(Machine_ID).Calculate(Current_Task);
                    if (Current_Task.Value /= -1) then
                        exit;
                    else
                        Service.Repair(Machine_Set(Current_Task.Op)(Machine_ID));
                        Print_Info_If_Talkative("WORKER: Put machine in service");
                    end if;
                end loop;
            else
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
    ...
    Current_Service_Worker_ID : Natural := 0;
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
begin
    Ceo.Start;
    ...
    Service.Start;
    for I in Service_Worker_Array'Range loop
        Service_Worker_Array(I).Start(Current_Service_Worker_ID);
        Current_Service_Worker_ID := Current_Service_Worker_ID + 1;
    end loop;    
